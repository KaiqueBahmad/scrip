...
const WITHDRAWAL_TONE = {
  pending: 'text-trace border-trace/30 bg-trace-soft',
  confirmed: 'text-settle border-settle/30 bg-settle-soft',
  denied: 'text-halt border-halt/30 bg-halt-soft',
} as const;

export function WithdrawalBadge({ status }: { status: 'pending' | 'confirmed' | 'denied' }) {
  const { t } = useTranslation();
  ...
  return (
    <span
      className={cn(
        'inline-flex items-center rounded-[var(--radius-panel)] border px-1.5 py-0.5',
        'font-mono text-[11px] font-medium whitespace-nowrap',
        WITHDRAWAL_TONE[status],
      )}
    >
      {t(`withdrawals.status${status.charAt(0).toUpperCase()}${status.slice(1)}`)}
    </span>
  );
}
